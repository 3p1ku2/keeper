...
async function propose(server, message) {
  ...
  if (cult.channels.proposals != message.channel.id) {
    message.reply(`ah <@${message.author.id}>, a cultist utters such things in #${server.getChannelName(cult.channels.proposals)}`)
    return
  }
