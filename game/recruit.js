...
const { lastChapterEndTime, chapterStartTime } = require('./state')
  ...
  if (user.referred_by && user.referred_by !== "") {
    ...
    if (zealot) {
      ...
      if (addOne) {
        ...
        // points.handleLeveling(zealotMember, referrals)
        // if (referrals.size >= 2 && !zealotMember.roles.cache.has(server.Roles.TrueBeliever)) {
...
async function handleSabotage(server, interaction) {
  ...
  await interaction.reply({ content: `your zealous link will now convert cultists to ${cult.getName(server)}`, ephemeral: true })
}
