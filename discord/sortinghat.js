...
async function handleChair(interaction) {
  if (server.isAdmin(interaction.member.id)) {
    await interaction.reply({content: 'admin cannot click. weird right?', ephemeral: true})
    return false
  }
  ...
  try {
    ...
    var cult = server.memberCult(member)
    if(!cult && cultist.cult_id && cultist.cult_id != ""){
      await interaction.editReply({ content: `you have already been assigned a cult, @keeper is working on it...`, ephemeral: true })
      return true
    }
    if (cult) {
      // already has cult
      try {
        await interaction.editReply({ content: `you are already in ${cult.getName(server)}`, ephemeral: true })
      } catch (err) {
        console.log(err)
      }
      return true
