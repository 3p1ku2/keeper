...
async function start(server, interaction) {
  if (interaction.channel.id != '986712037633720390') {
    await interaction.reply({ content: 'can\'t `/enter` here', ephemeral: true })
    return
  }
  let userCult = server.Cults.userCult(interaction.member)
  if (!userCult) {
    await interaction.reply({ content: 'join a cult before entering (hint: look for links that look like https://spells/quest/?z=a42xU79Oip7X in <#979919655105875999>)', ephemeral: true })
    return
  }
...
Actions you take earn points for your cult 𐂥.`)
    .addField('begin', `go to ${interaction.guild.channels.cache.get(server.channels.AltarChannelId)} and chant: ${userCult.chant}`)
...
async function continue_2(server, interaction) {
  ...
  if (_stats instanceof Error ) {
    await interaction.update({content:_stats, ephemeral:true})
  }
  let embed = new MessageEmbed()
    .setTitle("👝")
    .setColor("#FFFFE0")
...
async function cancel(server, interaction) {
  await interaction.reply({ content: 'you have exited', ephemeral: true })
}

async function _continue(server, interaction, step) {
  switch (step) {
    case 1:
      await continue_1(server, interaction)
      break
    case 2:
      await continue_2(server, interaction)
      break
    case 3:
      await continue_3(server, interaction)
      break
    case 4:
      await continue_4(server, interaction)
      break
  }
}
