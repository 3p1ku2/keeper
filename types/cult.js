...
class Cult {
  constructor(name, id, chant, emoji, discordEmoji, roleId, channels, emojiId, bonusPoints) {
    this.name = name
    this.id = id
    this.chant = chant
    this.emoji = emoji
    ...
    this.cache = {}
    Object.defineProperty(this, "cacheMutex", {
      enumerable: false,
      writable: true
    })
  ...
  async updateMetrics(server) {
    ...
    let agg = await server.db.collection("users").aggregate([
      {
        $match: {
          "cult_id": this.id,
          "history": { "$exists": true, "$type": 'array', "$not": { "$size": 0 }},
        }
      },
      {
        $group: {
    ...
    return this.cache.metrics.value
  }
  
  async getMetrics(server) {
    var release = await this.cacheMutex.acquire()
    try {
      if(this.cache.metrics){
        if (this.cache.metrics.timestamp +  20 * 1000 > Date.now()) {
          return this.cache.metrics.value
        }
      }
      return await this.updateMetrics(server)
    } finally {
      release()
    }
  }
...
class Cults {
  ...
  get(id) {
    for (const [key, cult] of Object.entries(this.cults)) {
      if (id == cult.id || id == cult.channels.main) {
        return cult
      }
  ...
  dungeonIds() {
    return this.values().map(cult => cult.channels.DungeonSectionId) 
  }
  ...
  proposalChannelIds() {
    let out = []
    for (const [key, cult] of Object.entries(this.cults)) {
      out.push(cult.channels.proposals)
    }
    return out
  }
