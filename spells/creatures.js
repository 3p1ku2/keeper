...
const DEFAULT_ATTACK_PERIOD = 60 * 60 * 1000
// const LIFESPAN = 4 * 60 * 60 * 1000
...
class Creature {
  constructor(strength, health, name, type) {
    ...
    this.attackPeriod = DEFAULT_ATTACK_PERIOD
    this.lastAttack = 0
    this.type = type
  }
...
async function handleDamage(server, creatureId, damage) {
  ...
  if(Number.isNaN(creature.healthRemaining)) {
...
async function runCreatures(server) {
  ...
  creatures.map(async (creature) => {
    ...
    if(Number.isNaN(creature.strengh)){
      killCreature(server, creature)
      return
    }
    //await handleDamage(server, creature.id, 0)
    if(creature.attackPeriod == 2 * 60 * 60 * 1000){
     creature.attackPeriod = DEFAULT_ATTACK_PERIOD
    }
