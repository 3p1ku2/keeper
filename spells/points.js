...
async function handleCastPoints(server, user, points) {
  ...
  console.log("points handleCastPoints user:", user.discord.userid)
...
async function loserboard(server, length) {
  ...
  let rows = await users.map(user => {
    ...
    var msg = `${i + 1} | ${_emoji} ${userTag(user)} • 𐂥 points: ${user.points ? user.points : 0} • recruits: ${user.referrals ? user.referrals.length : 0} • chants: ${user.num_chants}`
...
async function leaderboard(server, length) {
  ...
  let rows = await users.map(user => {
    ...
    var msg = `${i} | ${_emoji} ${userTag(user)} • 𐂥 points: ${user.points ? user.points : 0} • recruits: ${user.referrals ? user.referrals.length : 0} • chants: ${user.num_chants}`
    //  • <:magic:975922950551244871> ${user.coins ? user.coins : 0}
    i++
    return msg
  }).toArray()
...
function getReferralRankingMessage(users) {
  ...
  for (const user of users) {
    ...
    msg += `${i + 1} | ${_emoji} ${userTag(user)}  • recruits: ${user.num_referrals ? user.num_referrals : 0}\n`
    i++
  }
  if (msg.length > 0) {
...
function handleLeveling(user, referrals = null, totalChants = null){
  let member = server.getMember(user.discord.userid)
  if(!member){
    return
  }
  if(!referrals) {
    referrals = getAllPastReferralsSet(user)
  }
  if(!totalChants) {
    totalChants = getAllPastChantsCount(user)
  }
  let points = getAllPastPoints(user)
  
  if (points  > 0 && member.roles.cache.has(server.Roles.Unzealous)) {
    try {
  ...
  if(referrals.size >= 2 && !member.roles.cache.has(server.Roles.TrueBeliever)){
    try {
      member.roles.add(server.Roles.TrueBeliever)
    } catch(err){
      console.log("failed to add true believer role with error:", err)
    }
    adventure.log(server, `${member} is now a True Believer ${emoji.truebeliever}`)
  }
  if( (points >= 100 && totalChants > 3 && referrals.size >= 10) && !member.roles.cache.has(server.Roles.MindlessZealot)){
    try {
      member.roles.add(server.Roles.MindlessZealot)
    } catch (err) {
      console.log("failed to add mindless zealot role with error:", err)
    }
    adventure.log(server, `${member} is now a Mindless Zealot ${emoji.mindlesszealot}`)
  }
}
