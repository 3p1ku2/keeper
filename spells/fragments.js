...
async function handleReveal(interaction) {
  ...
  if (!cult) {
    await interaction.reply({ content: "error: no cult found for member", components: [], ephemeral: true })
    return
  }
  var piece = await userPiece(interaction.member.id, cult)
  if (!piece) {
    await interaction.reply({ content: "error: no piece found", components: [], ephemeral: true })
  }
