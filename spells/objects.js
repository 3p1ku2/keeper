...
var ChestMutex = new StringMutex()
class Chest {
  ...
  async handleUnlock(interaction) {
    if (this.recipients.includes(interaction.member.id) || interaction.member.id == this.summoner) {
      console.log("recipients includes user:", interaction.member.id, "recipients:", this.recipients)
      return false
    }
    this.recipients.push(interaction.member.id)
    await server.db.collection("objects").update({ "id": this.id }, { $set: { recipients: this.recipients } })
    if (this.recipients.length >= this.numUnlocksReqd) {
      await this.commit()
      await interaction.editReply({ content: 'chest unlocked!', ephemeral: true })
    } else {
      await this.updateMsg()
      await interaction.editReply({ content: '🗝️ accepted!', ephemeral: true })
    }
    return true
  }
  ...
  async updateMsg() {
    ...
    let updatedMsg = new MessageEmbed()
      ...
      .setDescription(`get ${this.numUnlocksReqd} 🗝 clicks before <t:${Math.floor(this.end.getTime() / 1000)}:f> to unlock and get loot`)
    ...
    let message = await channel.send({ embeds: [updatedMsg], components: [
      new MessageActionRow()
        .addComponents(
          new MessageButton()
            .setCustomId('unlock_chest')
            .setLabel('← click here to unlock (reactions not counted)')
            .setEmoji('🗝️')
            .setStyle('PRIMARY')
        )
    ] })
    this.clientAppId = server.client.application.id
  ...
  async issueReward() {
    ...
    switch (_reward.value) {
      case "magic":
        ...
        var users = []
        if(this.recipients.length > this.numUnlocksReqd){
          this.recipients.splice(this.numUnlocksReqd, this.recipients.length-1)
        }
  ...
  async commit() {
    ...
    if (this.messageId) {
      ...
      if (msg) {
        try {
          await msg.edit({ embeds: [updatedMsg], components:[] })
        } catch(err){
          console.log("error editing msg:", err)
        }
...
async function createChest(power, targetCult, creatorId) {
  ...
  chest.channelId = targetCult.channels.chests
...
async function handleUnlock(interaction){
  ...
  console.log("chest message id:", interaction.message.id)
  var release = await ChestMutex.acquire(interaction.message.id)
  console.log("chest message id:", interaction.message.id, "loaded mutex")
  try{
    let object = await server.db.collection("objects").findOne({ messageId: interaction.message.id, state: "active" })
    if (!object) {
    ...
    setPrototype(object)
    try {
      let success = await object.handleUnlock(interaction)
      return success
    } catch (err) {
