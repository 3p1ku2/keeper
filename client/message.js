...
async function handleChant(msg, noReply) {
  ...
  if (msg.channel.id != server.channels.AltarChannelId) {
    msg.reply("the altar has moved, chant in " + msg.guild.channels.cache.get(server.channels.AltarChannelId).toString())
    return
  }
...
async function handleBound(msg){
...
async function handle(msg) {
  ...
  if (server.isAdmin(msg.author.id)) {
    if (msg.content.startsWith("!logChant")) {
      ...
      forceCountMessage(server.channels.AltarChannelId, messageId)
      return
    }

    if (msg.content.startsWith("!logMissedChants")) {
      let from = msg.content.replace("!logMissedChants ", "")
      logMissedChants(server.channels.AltarChannelId, from)
      return
    }
